...
beforeAll(async () => {
  ...
  const users = await app.db('users').insert([
    { name: 'User #1', mail: 'user@mail.com', passwd: '$2a$10$.WnOauBV9T.aNiOxFpCIBeq639ADuaVT3WOHaNIglWTLq3tNzeM4a' },
    { name: 'User #2', mail: 'user2@mail.com', passwd: '$2a$10$.WnOauBV9T.aNiOxFpCIBeq639ADuaVT3WOHaNIglWTLq3tNzeM4a' },
  ], '*');
  [user, user2] = users;
  delete user.passwd;
...
describe('Ao tentar inserir uma transação inválida', () => {
  // const validTransaction = { description: 'New T', date: new Date(), ... };
  // outra opção é colocar diretamente no template
...
test('Deve alterar uma transação por ID', () => {
  return app.db('transactions').insert({ description: 'to Update', date: new Date(), ammount: 100, type: 'I', acc_id: accUser.id }, ['id'])
